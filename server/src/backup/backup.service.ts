  ...
  async createAndSendBackup(): Promise<void> {
    ...
    if (fullZipBuffer.length <= this.MAX_ATTACHMENT_SIZE) {
      ...
      await this.transporter.sendMail({
        ...
        to: adminEmail,
        subject: `🗄️ MyHunter Database Backup - ${date}`,
        html: `
          <h2>MyHunter Database Backup</h2>
        ...
        attachments: [
          {
            filename: `myhunter-backup-${date}.zip`,
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        await this.transporter.sendMail({
          from: process.env.EMAIL_FROM || 'no-reply@example.com',
          to: adminEmail,
          subject: `🗄️ MyHunter Backup - ${date} [ნაწილი ${i + 1}/${parts.length}]`,
          html: `
            <h2>MyHunter Database Backup</h2>
            <p><strong>თარიღი:</strong> ${date}</p>
          ...
          attachments: [
            {
              filename: `myhunter-backup-${date}-part${i + 1}.zip`,
